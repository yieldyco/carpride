		...
		if ($product_info) {
			...
			if (!$json) {
				...
				if ($this->config->get('remarketing_status')) {
					...
					$json['remarketing'] = $this->model_tool_remarketing->remarketingAddToCart($product_info, $quantity, !empty($options) ? $options : '', $option); $json['remarketing']['vcart'] = '619525693732002';
				}
			...
			if ($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot()) {
				...
				foreach ($this->cart->getProducts() as $product) {
					if ($product['cart_id'] == $this->request->post['key']) {
						$product_info = $this->model_catalog_product->getProduct($product['product_id']); $product_info['price'] = $product['price']; $product_info['options'] = $product['option']; $quantity = $product['quantity'];
					} 
				}
			}
			$this->cart->remove($this->request->post['key']);
			if ($this->config->get('remarketing_status') && !empty($product_info) && !empty($quantity)) {
