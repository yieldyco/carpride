...
// remarketing all in one
	    ...
	    if ($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot()) {
			$this->load->model('catalog/product'); $product_info = [];
			foreach ($this->cart->getProducts() as $product) {
				if ($product['cart_id'] == $this->request->request['remove']) {
					$product_info = $this->model_catalog_product->getProduct($product['product_id']); $quantity = $product['quantity'];	$product_info['price'] = $product['price'];	$product_info['options'] = $product['option'];
				} 
			}
		}
            $this->cart->remove($this->request->request['remove']);
if ($this->config->get('remarketing_status')  && !empty($product_info) && !empty($quantity) && !$this->model_tool_remarketing->isBot()) {
