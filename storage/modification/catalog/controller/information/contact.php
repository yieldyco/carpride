...
class ControllerInformationContact extends Controller {
	...
	public function index() {
			...
			if (isset($oct_deals_data['contact_email']) && strlen($oct_deals_data['contact_email']) > 3) {
				$oct_deals_data['main_contact_email'] = $oct_deals_data['contact_email'];
			}
			...
			$oct_contact_opens = explode(PHP_EOL, $oct_deals_data['contact_open'][(int)$this->config->get('config_language_id')]);
				...
				foreach ($oct_contact_opens as $oct_contact_open) {
					if (!empty($oct_contact_open)) {
						$oct_deals_data['oct_contact_opens'][] = html_entity_decode($oct_contact_open, ENT_QUOTES, 'UTF-8');
					}
				}
			}
			...
			if ((isset($oct_deals_data['contact_view_phone']) && $oct_deals_data['contact_view_phone'] == 'on') && (isset($oct_deals_data['contact_telephone']) && !empty($oct_deals_data['contact_telephone']))) {
				...
				foreach ($oct_contact_telephones as $oct_contact_telephone) {
					if (!empty($oct_contact_telephone)) {
						...
						$oct_deals_data['oct_contact_telephones'][] = html_entity_decode(trim($oct_contact_telephone), ENT_QUOTES, 'UTF-8');
					}
				}
			}
			...
			if (isset($oct_deals_data['contact_view_locations']) && $oct_deals_data['contact_view_locations'] == 'on') {
				...
				$data['oct_locations_microdata'] = $this->model_octemplates_microdata->generateLocationsOrganizationMicrodata($data['oct_locations']);
			}
			...
			$oct_deals_data['oct_micro'] = true;
			$data['oct_microdata'] = $this->model_octemplates_microdata->generateHomeMicrodata($oct_deals_data);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if ($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot() && $this->config->get('remarketing_telegram_status') && $this->config->get('remarketing_telegram_contact_form')) {
				$this->load->model('tool/remarketing_core'); $this->model_tool_remarketing_core->sendTelegramMsg('Contact Form' . "\n\n" . $this->request->post['name'] . "\n" . $this->request->post['email'] . "\n\n" . $this->request->post['enquiry']);
			}
