...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if ($this->config->get('remarketing_status')) { $this->session->data['remarketing_order_id'] = $order_id; setcookie('remarketing_order_id', $order_id, time() + 24 * 3600, '/'); $this->load->model('tool/remarketing_core'); $this->model_tool_remarketing_core->getOrderRemarketing($order_id); }
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			$this->session->data['process_order'] = true;
			if ($this->config->get('remarketing_status')) {
				$this->load->model('tool/remarketing_core');
				$this->model_tool_remarketing_core->processOrder($order_id, $order_status_id, $order_info);
				unset($this->session->data['process_order']); 
			}
