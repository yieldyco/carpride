...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		$q = $this->db->query("DESCRIBE  " . DB_PREFIX . "product_to_category `main_category`");
		if ($q->num_rows == 0) {
			$sql = "ALTER TABLE `" . DB_PREFIX . "product_to_category` ADD `main_category` tinyint(1) NOT NULL DEFAULT '0' AFTER `category_id`";
			$query = $this->db->query($sql);
		}  

		$q = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url WHERE `query` = 'common/home' AND `language_id` = " . (int)$this->config->get('config_language_id') . " AND `store_id` = 0");
		if ($q->num_rows == 0) {
			$sql = "INSERT INTO `" . DB_PREFIX . "seo_url` (`store_id`, `language_id`, `query`, `keyword`) VALUES (0, " . (int)$this->config->get('config_language_id') . ", 'common/home', '')";
			$query = $this->db->query($sql);
		} 
		...
		if (isset($this->request->post['config_seopro_addslash_product'])) {
			$data['config_seopro_addslash_product'] = $this->request->post['config_seopro_addslash_product'];
		} elseif ($this->config->has('config_seopro_addslash_product')) {
			$data['config_seopro_addslash_product'] = $this->config->get('config_seopro_addslash_product');
		}
