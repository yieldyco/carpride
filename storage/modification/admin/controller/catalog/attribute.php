...
class ControllerCatalogAttribute extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->load->model('catalog/attribute_text_select'); $filter_get = $this->model_catalog_attribute_text_select->filterGet();
            if(!empty($filter_get['sort_filter'])){if(empty($url)){$url='';}$url.=$filter_get['sort_filter'];}
	...
	protected function getList() {
        ...
        */ /* next attribute_text_select*/
        $data['status_attribute_text'] = null;
        $this->load->model('catalog/attribute_text_select');
        if($this->model_catalog_attribute_text_select->isStatusModule()) {
            $res_filter = $this->model_catalog_attribute_text_select->getAttributesFilter($filter_data);
            foreach($res_filter as $pole => $val) {
                if($pole == 'results') {$results = $val;}
                elseif($pole == 'attribute_total') {$attribute_total = $val;}
                else {$data[$pole] = $val;}
            }
            if(!empty($data['sort_filter'])){if(empty($url)){$url='';}$url.=$data['sort_filter'];}
        }
        else {
		...
		foreach ($results as $result) {
			$data['attributes'][] = array(
                ...
                /*attribute_text_select*/
                'count_prod' => isset($result['count_prod']) ? $result['count_prod'] : 0,
                /*end attribute_text_select*/
        ...
        /*attribute_text_select*/ if(!empty($data['sort_filter'])){$url.=$data['sort_filter'];} /*end attribute_text_select*/
        ...
        /*attribute_text_select*/
        $token_token = $this->model_catalog_attribute_text_select->tokenUser();
        $data['sort_count_prod'] = $this->url->link('catalog/attribute', $token_token.'&sort=count_prod'.$url, true);
        /*end attribute_text_select*/
	...
	protected function getForm() {
        ...
        /*attribute_text_select*/
        $data['status_attribute_text'] = 0;
        if(isset($this->request->get['attribute_id']) && ($attribute_id = (int)$this->request->get['attribute_id'])) {
            $this->load->model('catalog/attribute_text_select');
            if($this->model_catalog_attribute_text_select->isStatusModule()) {
                if(empty($url)) {$url = '';}
                $res_form_attr = $this->model_catalog_attribute_text_select->getAttribTextForm($attribute_id, $url);
                foreach($res_form_attr as $pole => $val) {$data[$pole] = $val;}
            }
        }
        /*end attribute_text_select*/
    ...
    public function autocomplete_attrb() {
		$this->load->model('catalog/attribute_text_select'); $json = $this->model_catalog_attribute_text_select->autoCompleteAttribFilter();
        $this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	}
    public function ajs_post() {
        $json = array(); $error_warning = ''; $redir = '';
        if(isset($this->request->get['attribute_id']) && ($attribute_id = (int)$this->request->get['attribute_id'])) {
            $data = array(); $this->load->model('catalog/attribute_text_select');
            if(isset($_POST['add_param']) && $this->validateAttribute()) {
                $result = $this->model_catalog_attribute_text_select->addAttribText($attribute_id);
                if(isset($result['duble'])) {$json['duble'] = $result['duble'];$json['t_id'] = $result['t_id'];}
                elseif(isset($result['t_id'])) {$json['succ'] = $result['succ'];$json['t_id'] = $result['t_id'];$json['add'] = 1;$redir = 1;} else {$error_warning = 'ERROR json: ';}
            }
            elseif(isset($_POST['del_values']) && $this->validateAttribute()) {
                $data['attribute_id'] = $attribute_id;
                if(isset($_POST['del_selected_text_id'])) {$data['selected_text_id'] = $this->request->post['del_selected_text_id'];}
                elseif(isset($_POST['del_param'])) {$json['del_param'] = $data['text_id'] = $text_id = (int)$this->request->post['del_param'];}
                $result = $this->model_catalog_attribute_text_select->delSelectedValues($data);
                if(isset($result['succ'])) {$json['succ'] = $result['succ'];$json['del_values'] = 1;$redir = 1;} 
                else {$error_warning = 'ERROR json: ';}
            }
        }
        else {$error_warning = 'no attribute_id';}
	...
	protected function validateDelete() {
		...
		/*attribute_text_select*/ $this->load->model('catalog/attribute_text_select'); if($this->model_catalog_attribute_text_select->isStatusModule()) {return !$this->error;} /*end attribute_text_select*/
