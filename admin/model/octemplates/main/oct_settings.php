...
class ModelOCTemplatesMainOctSettings extends Model {
	...
	public function addOCTMenu($data = []) {
		if ($data) {
			...
			if ($menu_ids) {
				...
				$this->db->query("DELETE FROM `" . DB_PREFIX . "oct_menu_sub_links` WHERE `oct_menu_id` NOT IN (". $menu_id .")");
				$this->db->query("DELETE FROM `" . DB_PREFIX . "oct_menu_sub_links_description` WHERE `oct_menu_id` NOT IN (". $menu_id .")");
			}
			...
			foreach ($data as $menu) {
				...
				if (isset($menu['sub_links'])) {
					$this->db->query("DELETE FROM `" . DB_PREFIX . "oct_menu_sub_links` WHERE `oct_menu_id` = '" . (int)$oct_menu_id . "'");
					$this->db->query("DELETE FROM `" . DB_PREFIX . "oct_menu_sub_links_description` WHERE `oct_menu_id` = '" . (int)$oct_menu_id . "'");
				}
				...
				if (isset($menu['sub_links']) && is_array($menu['sub_links'])) {
					foreach ($menu['sub_links'] as $sub_link_id => $sub_link) {
						$sort_order = isset($sub_link['sort_order']) ? (int)$sub_link['sort_order'] : 0;
						
						$this->db->query("INSERT INTO `" . DB_PREFIX . "oct_menu_sub_links` SET `oct_menu_id` = '" . (int)$oct_menu_id . "', `sub_link_id` = '" . $this->db->escape($sub_link_id) . "', `sort_order` = '" . (int)$sort_order . "'");

						if (isset($sub_link['description']) && is_array($sub_link['description'])) {
							foreach ($sub_link['description'] as $language_id => $description) {
								$title = isset($description['title']) ? $description['title'] : '';
								$href = isset($description['href']) ? $description['href'] : '';
								
								$this->db->query("INSERT INTO `" . DB_PREFIX . "oct_menu_sub_links_description` SET `oct_menu_id` = '" . (int)$oct_menu_id . "', `sub_link_id` = '" . $this->db->escape($sub_link_id) . "', `language_id` = '" . (int)$language_id . "', `title` = '" . $this->db->escape($title) . "', `href` = '" . $this->db->escape($href) . "'");
							}
						}
					}
				}
			...
			$this->db->query("TRUNCATE `" . DB_PREFIX . "oct_menu_sub_links`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "oct_menu_sub_links_description`");
		}
	}
    ...
    public function getMenuItems() {
        ...
        foreach ($query->rows as $menu_item) {
            $description = $customers = $stories = $sub_links = [];
            ...
            $query_sub_links = $this->db->query("SELECT * FROM `" . DB_PREFIX . "oct_menu_sub_links` WHERE `oct_menu_id` = '" . (int)$menu_item['oct_menu_id'] . "' ORDER BY `sort_order`");

            foreach ($query_sub_links->rows as $sub_link) {
                $sub_link_description = [];
                
                $query_sub_description = $this->db->query("SELECT * FROM `" . DB_PREFIX . "oct_menu_sub_links_description` WHERE `oct_menu_id` = '" . (int)$menu_item['oct_menu_id'] . "' AND `sub_link_id` = '" . $this->db->escape($sub_link['sub_link_id']) . "'");

                foreach ($query_sub_description->rows as $descr) {
                    $sub_link_description[$descr['language_id']] = [
                        'title' => $descr['title'],
                        'href' => $descr['href'],
                    ];
                }

                $sub_links[$sub_link['sub_link_id']] = [
                    'description' => $sub_link_description,
                    'sort_order' => $sub_link['sort_order'],
                ];
            }
    ...
    public function installOCTMenuSubLinks() {
        $sqls[] = "
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "oct_menu_sub_links` (
              `oct_menu_id` int(11) NOT NULL,
              `sub_link_id` varchar(50) NOT NULL,
              `sort_order` int(3) NOT NULL DEFAULT '0',
              PRIMARY KEY (`oct_menu_id`, `sub_link_id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
        ";

        $sqls[] = "
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "oct_menu_sub_links_description` (
              `oct_menu_id` int(11) NOT NULL,
              `sub_link_id` varchar(50) NOT NULL,
              `language_id` int(11) NOT NULL,
              `title` varchar(255) NOT NULL,
              `href` varchar(255) NOT NULL,
              PRIMARY KEY (`oct_menu_id`, `sub_link_id`, `language_id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
        ";
	...
	public function checkAndInstallMenuSubLinks() {
		$existing_tables = [];
		...
		if (!in_array(DB_PREFIX . 'oct_menu_sub_links', $existing_tables) || 
			!in_array(DB_PREFIX . 'oct_menu_sub_links_description', $existing_tables)) {
			$this->installOCTMenuSubLinks();
		}
	}
    ...
    public function deleteOCTMenu() {
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oct_menu`");
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oct_menu_description`");
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oct_menu_to_customer_group`");
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oct_menu_to_store`");
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oct_menu_sub_links`");
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oct_menu_sub_links_description`");
    }

    public function deleteOCTLocation() {
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "oct_location`");
    }
