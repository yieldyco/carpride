...
class ModelOCTemplatesMenuOCTMenu extends Controller {
    ...
    public function getOCTMenuItems($data = []) {
        ...
        if ($inType) {
            ...
            foreach ($query->rows as $oct_item) {
                ...
                $sub_links = $this->getMenuSubLinks($oct_item['oct_menu_id']);
    ...
    private function getMenuSubLinks($oct_menu_id) {
        $sub_links = [];
        ...
        $query = $this->db->query("
            SELECT 
                ...
                sld.title,
                sld.href
            FROM `" . DB_PREFIX . "oct_menu_sub_links` sl
            LEFT JOIN `" . DB_PREFIX . "oct_menu_sub_links_description` sld
                ON (sl.oct_menu_id = sld.oct_menu_id AND sl.sub_link_id = sld.sub_link_id)
            WHERE 
                sl.oct_menu_id = '" . (int)$oct_menu_id . "'
                AND sld.language_id = '" . (int)$this->config->get('config_language_id') . "'
            ORDER BY sl.sort_order ASC
        ");
        
        foreach ($query->rows as $sub_link) {
            $sub_links[] = [
