...
class ModelExtensionPaymentMonoPay extends Model {
    ...
    public function getMethod($address, $total)
    {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_mono_pay_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if (($this->config->get('payment_mono_pay_total') > $total) || ($this->config->get('payment_mono_pay_total_max') > 0 && ($this->config->get('payment_mono_pay_total_max') < $total))) {
            $status = false;
        } elseif (!$this->config->get('payment_mono_pay_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
			$lang = $this->config->get('payment_mono_pay_description');
			
			if(isset($lang[$this->config->get('config_language_id')]['name']) && !empty($lang[$this->config->get('config_language_id')]['name'])) {
				$title = $lang[$this->config->get('config_language_id')]['name'];
			} else {
            ...
            $method_data = array(
                ...
                'title' 		=> $title,
                'terms'			=> '',
                'sort_order' 	=> $this->config->get('payment_mono_pay_sort_order')
            );
        }
